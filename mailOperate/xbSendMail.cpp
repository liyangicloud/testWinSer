...
CSmtp::CSmtp(void)
{
    ...
    this->sockClient = INVALID_SOCKET;
	m_bSocketUP = false;
...
bool CSmtp::CreateConn()
{
	...
	m_bSocketUP = true;//到此表示socket初始化成功，如果退出必须进行清理操作
...
int CSmtp::SendAttachment_Ex() /*发送附件*/
{
    for (std::list<FILEINFO *>::iterator pIter = listFile.begin(); pIter != listFile.end(); pIter++)
    {
        ...
        {
            //cout << "文件传送出错!" << endl;
            return XB_ERRORCODE_ATTACHFILE_FAILED;
        }
        delete[]chSendBuff;
    }
    return 0;
...
int CSmtp::SendEmail_Ex()
{
	...
	lys("head sending  success!\n");
    if (false == SendTextBody())
    {
        return 1; /*错误码1是由于网络的错误*/
    }
	lys("body sending success!\n");
    err = SendAttachment_Ex();
    if (err != 0)
    {
        return err;
    }
	lys("attachments success!\n");

    if (false == SendEnd())
    {
        return 1; /*错误码1是由于网络的错误*/
    }
	lys("mail's end sending success!\n");
